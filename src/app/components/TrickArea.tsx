...
export function TrickArea({ currentTrick, playerCount, currentTurnIndex, playerNames, playerTeams, size = 400, localSeatIndex = 0 }: TrickAreaProps) {
  ...
  // Keep seats further from center so name labels don't overlap cards
  const avatarDist  = Math.round(172 * r); // seat indicator distance from center

  // Dynamic sizing based on table scale
  const avatarPx    = Math.max(36, Math.round(48 * r));  // avatar circle diameter
  const avatarEmoji = Math.max(16, Math.round(22 * r));  // emoji inside avatar
          ...
          <div key={`seat-${i}`} className="absolute" style={{
            ...
            <div className="flex flex-col items-center" style={{ gap: 4 }}>
              <div className="relative">
                ...
                {/* Avatar circle */}
                <div
                  className={`rounded-full flex items-center justify-center transition-all duration-300 ${isCurrent ? 'animate-turn-ring' : ''}`}
                  style={{
                    width: avatarPx,
                    height: avatarPx,
