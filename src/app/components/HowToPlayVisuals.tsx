...
const TEAM_BG = ['rgba(37,201,245,0.15)', 'rgba(255,107,138,0.15)'];
const TEAM_BD = ['rgba(37,201,245,0.4)', 'rgba(255,107,138,0.4)'];
const TEAM_TX = ['#25C9F5', '#FF6B8A'];
const AVATARS = ['🦁', '🦅', '🐘', '🦚', '🐅', '🐍', '🦎', '🐎'];
...
function MiniCard({ rank, suit, isMindi, dim, back, w = 42, h = 58 }: MiniCardProps) {
  ...
  if (back) return (
    <div style={{
      ...
      background: 'linear-gradient(160deg,#7B5CFF,#5B37B0,#40287F)',
      border: '1px solid rgba(255,201,60,0.25)',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      fontSize: 14, filter: dim ? 'brightness(0.5) grayscale(0.4)' : undefined,
    }}>
      <span style={{ color: 'rgba(255,201,60,0.35)', fontSize: 16 }}>♠</span>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      border: isMindi ? '1.5px solid rgba(255,201,60,0.65)' : '1px solid rgba(0,0,0,0.1)',
      boxShadow: isMindi ? '0 0 10px rgba(255,201,60,0.45)' : '0 2px 6px rgba(0,0,0,0.25)',
      ...
      {isMindi && (
        <div style={{
          ...
          background: 'radial-gradient(ellipse at 50% 60%,rgba(255,201,60,0.18),transparent 70%)',
        }} />
      )}
    </div>
  );
...
function PlayerBadge({ name, teamId, isCurrent, avatar }: { name: string; teamId: 0|1; isCurrent?: boolean; avatar: string }) {
  return (
    <div style={{
      ...
      background: isCurrent ? 'rgba(255,201,60,0.15)' : TEAM_BG[teamId],
      border: `1px solid ${isCurrent ? 'rgba(255,201,60,0.5)' : TEAM_BD[teamId]}`,
      color: isCurrent ? '#FFC93C' : TEAM_TX[teamId],
      boxShadow: isCurrent ? '0 0 8px rgba(255,201,60,0.3)' : undefined,
    }}>
      <span style={{ fontSize: 12 }}>{avatar}</span>
      <span>{name}</span>
      {isCurrent && <span style={{ fontSize: 7, color: '#FFC93C' }} className="animate-pulse">●</span>}
    </div>
  );
}
...
function ObjectiveVisual() {
  ...
  return (
    <div style={{
      ...
      background: 'linear-gradient(168deg,#40287F,#2C1F5E)',
      ...
      <div style={{
        ...
        borderBottom: '1px solid rgba(255,201,60,0.1)',
      }}>
        <span style={{ color: '#FFC93C', fontSize: 12 }}>♠</span>
        <span style={{ fontFamily: 'serif', letterSpacing: '0.15em', fontSize: 9, color: 'rgba(255,201,60,0.6)' }}>MINDI</span>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: 8 }}>
        <div style={{
          ...
          background: 'rgba(255,201,60,0.12)', border: '1px solid rgba(255,201,60,0.3)', color: '#FFC93C',
        }}>
    ...
    <div style={{
      ...
      <div style={{ position: 'relative', height: 130 }}>
        ...
        <div style={{
          ...
          width: 130, height: 75,
          borderRadius: 16,
          background: 'radial-gradient(ellipse at 50% 40%,rgba(255,201,60,0.04),rgba(0,0,0,0.4))',
          border: '1px solid rgba(255,201,60,0.1)',
        }}>
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <span style={{ fontSize: 8, color: 'rgba(255,201,60,0.15)', letterSpacing: '0.1em' }}>TABLE</span>
          </div>
        </div>
        {/* Top — Arjun Team A */}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 6 }}>
        ...
        <div style={{ flex: 1 }} />
        <div style={{ padding: '2px 6px', borderRadius: 6, fontSize: 8, background: 'rgba(255,201,60,0.08)', border: '1px solid rgba(255,201,60,0.2)', color: '#FFC93C' }}>
          Trick #3
        </div>
      </div>
      <div style={{ position: 'relative', height: 120 }}>
        ...
        <div style={{
          ...
          background: 'radial-gradient(circle at 40% 35%,rgba(255,201,60,0.05),rgba(30,8,8,0.5))',
          border: '1px solid rgba(255,201,60,0.1)',
        }} />
        {/* Cards */}
        {played.map((p, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              filter: p.winner ? 'drop-shadow(0 0 8px rgba(255,201,60,0.6))' : undefined,
              zIndex: p.winner ? 2 : 1,
            }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: 4 }}>
        <div style={{
          ...
          background: 'rgba(255,201,60,0.08)', border: '1px solid rgba(255,201,60,0.25)', color: '#FFC93C',
        }}>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(255,201,60,0.12)',
      }}>
        <span style={{ fontFamily: 'serif', fontSize: 10, color: 'rgba(255,201,60,0.6)', letterSpacing: '0.2em' }}>MINDI</span>
        <div style={{
          display: 'flex', alignItems: 'center', gap: 5,
          padding: '3px 10px', borderRadius: 99,
          background: 'rgba(255,201,60,0.08)', border: '1px solid rgba(255,201,60,0.2)',
        }}>
          <span style={{ fontSize: 10, color: '#FFC93C' }}>⚡</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 8, color: '#FFC93C', marginTop: 4 }}>Trump 2</div>
        </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'serif', fontSize: 14, fontWeight: 700, color: '#FFC93C', letterSpacing: '0.15em' }}>MENDIKOT!</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 6, alignItems: 'center' }}>
        ...
        <div style={{
          ...
          background: 'rgba(37,201,245,0.08)', border: '1px solid rgba(37,201,245,0.25)',
        }}>
          <div style={{ fontSize: 8, color: 'rgba(37,201,245,0.6)', letterSpacing: '0.1em', marginBottom: 2 }}>TEAM A</div>
          <div style={{ fontFamily: 'serif', fontSize: 22, fontWeight: 700, color: '#25C9F5', lineHeight: 1 }}>7</div>
          <div style={{ fontSize: 7, color: 'rgba(37,201,245,0.4)', marginTop: 1 }}>4 Mindis · 10 tricks</div>
        </div>
        {/* VS */}
        <div style={{ textAlign: 'center' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 8, color: 'rgba(255,107,138,0.6)', letterSpacing: '0.1em', marginBottom: 2 }}>TEAM B</div>
          <div style={{ fontFamily: 'serif', fontSize: 22, fontWeight: 700, color: '#FF6B8A', lineHeight: 1 }}>4</div>
          <div style={{ fontSize: 7, color: 'rgba(255,107,138,0.4)', marginTop: 1 }}>0 Mindis · 5 tricks</div>
        </div>
      </div>
      {/* Progress bar */}
      <div style={{ marginTop: 8, borderRadius: 99, overflow: 'hidden', height: 4, background: 'rgba(255,255,255,0.05)' }}>
        <div style={{ height: '100%', width: '63%', borderRadius: 99, background: 'linear-gradient(90deg,#25C9F5,#4a90d4)' }} />
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 3 }}>
        <span style={{ fontSize: 7, color: 'rgba(37,201,245,0.4)' }}>Team A — 7/11 pts</span>
